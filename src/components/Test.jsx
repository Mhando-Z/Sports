...
/**
 * ShakaPlayer
 * Production-grade IPTV player built on shaka-player.
 *
 * Handles arbitrary incoming stream URLs (HLS / DASH, live or VOD),
 * with ABR quality control, network resilience, a live bitrate/signal
 * HUD, and a fully custom glassmorphic control surface (no native
 * <video> controls — IPTV is typically driven by remote/touch on
 * large screens, so big hit targets and high contrast matter more
 * than native browser chrome).
 *
 * Requires shaka-player to be loaded globally as `window.shaka`
 * (e.g. via the official CDN script, or `import shaka from "shaka-player/dist/shaka-player.ui.js"`).
 */
...
export default function ShakaPlayer({ src }) {
  ...
  const refreshTracks = useCallback(() => {
    ...
    try {
      ...
      setActiveTrackId(active ? active.id : null);
    } catch {
  ...
  useEffect(() => {
    ...
    const initPlayer = async () => {
      ...
      if (typeof window === "undefined" || !window.shaka) {
        setStatus("error");
        setErrorMsg("Shaka Player library failed to load.");
        return;
      }
      ...
      if (!shaka.Player.isBrowserSupported()) {
        setStatus("error");
        setErrorMsg("This browser does not support video playback.");
        return;
      }
      ...
      player.configure({
        streaming: {
          // Buffering tuned for unstable IPTV / live network conditions
          bufferingGoal: 60,
          rebufferingGoal: 4,
          bufferBehind: 30,
          retryParameters: {
          ...
          // Recover from live-edge stalls automatically
          jumpLargeGaps: true,
          smallGapLimit: 1.5,
          // Live streams: stay close to the live edge
        ...
        manifest: {
          ...
          dash: {
            // Be tolerant of slightly out-of-spec IPTV manifests
      ...
      // Network request filter: helps with IPTV CDNs that need
      // permissive headers / redirects handled gracefully.
